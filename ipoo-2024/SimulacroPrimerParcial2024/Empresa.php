...
class Empresa{
    ...
    /**
     * Verifica si el cliente esta o no habilitado para realizar compras
     * baja = true ---> no habilitado
     * baja = false ---> habilitado
     * @param INT $numDNI
     * @return 
     */
    /*public function verificaCliente($numDNI){
        $colC = $this->getColCliente();

        //Uso while porque corta cuando encuentra
        $encontro = false;
        $i = 0;
        while($i < count($colC) && !$encontro){
            if($numDNI == $colC[$i]->getNroDNI()){
                if($colC[$i]->getBaja()){
                    $encontro = true; //Aca encuentra a un pasajero que esta dado de baja
                }
            }
            $i++;
        }
    ...
    /**
     * Verifica si la moto no esta disponible para ser vendida
     * activa = true ---> no disponible
     * activa = false ---> disponible
     * @param INT $codigo
     * @return 
     */
    /*public function verificaMoto($codigo){
        $colM = $this->getColMoto();
        ...
        $i = 0;
        while($i < count($colM) && !$encontro){
            if($codigo == $colM[$i]->getCodigo()){
                if($colM[$i]->getActiva()){
                    $encontro = true; //Aca encuentra a una moto que esta dada de baja
                }
            }
            $i++;
        }
    ...
    public function retornarVentasXCliente($tipo,$numDoc){
        $colObjVentas=$this->getColVenta();
        $i=0;
        $encontrado=false;
        $ventaRealizada="";
        while($i<count($colObjVentas) && !$encontrado){
            $venta=$colObjVentas[$i]->getObjCliente();
            $tipoDNI=$venta->getTipoDNI();
            $nroDNI=$venta->getNroDNI();
            if($tipoDNI==$tipo && $numDoc==$nroDNI){
                $ventaRealizada=$colObjVentas[$i];
                $encontrado=true;
            }
            $i++;
        }

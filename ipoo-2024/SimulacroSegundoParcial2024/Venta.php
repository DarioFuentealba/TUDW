...
class Venta{
    ...
    public function __toString(){
        $msjToString = "\n\nN° de venta TSV: ".$this->getNro()."\nFecha en que se realizó la venta TSV: ".$this->getFecha()."\nDatos del cliente que compró la moto TSV: ".$this->getObjCliente()."\nDatos de las motos TSV: ".$this->datosMotos()."\nPrecio final TSV: ".$this->getPrecioFinal()."\n\n";
     ...
     * @return FLOAT $sumaPrecioVentaMotosNacionales
     */
    public function retornarTotalVentaNacional(){
        $colM = $this->getColMoto();
        $sumaPrecioVentaMotosNacionales = 0;

        for($i = 0; $i < count($colM); $i++){
            if($colM[$i] instanceof MotoNacional){
                $precioVenta = $colM[$i]->darPrecioVenta();
                $sumaPrecioVentaMotosNacionales = $sumaPrecioVentaMotosNacionales + $precioVenta;
            }
        }
        return $sumaPrecioVentaMotosNacionales;
    }
     ...
     * @return ARRAY $arrayMotosImportadasVendidas
     */
    public function retornarMotosImportadas(){
        $colM = $this->getColMoto();
        $arrayMotosImportadasVendidas = [];

        //Uso for porque se cuantos ciclos hara
        for($i = 0; $i < count($colM); $i++){
            if($colM[$i] instanceof MotoImportada){
                array_push($arrayMotosImportadasVendidas, $colM[$i]);
            }
        }
        return $arrayMotosImportadasVendidas;
    }
}
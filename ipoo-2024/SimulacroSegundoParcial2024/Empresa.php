...
class Empresa{
    ...
    public function __toString(){
        ...
        $msjToString = "\n\nDenominación de la empresa TSE: ".$this->getDenominacion()."\nDirección de la empresa TSE: ".$this->getDireccion()."\nClientes de la empresa TSE: \n".$this->recorrerObjectos($objCliente)."\nMotos de la empresa TSE: \n".$this->recorrerObjectos($objMoto)."\nVentas de la empresa TSE: \n".$this->recorrerObjectos($objVenta)."\n\n";
     ...
     * (*IMPORTANTE: invocar a los métodos implementados en la clase venta cuando crea necesario)
     * @return FLOAT $impoteTotalVentasNacionales
     */
    public function informarSumaVentasNacionales(){
        $colV = $this->getColVenta();
        $precioVentasMotosNacionalesDeTodasLasVentasQueSeHicieron = 0;

        //Uso for porque se cuantos ciclos hara
        for($i = 0; $i < count($colV); $i++){
            if($colV[$i]->retornarTotalVentaNacional()){
                /*
                todo: Aca que hago? Le estoy diciendo si es true o le digo que tome el valor que retorna esa funcion?
                */
                $precioVentasMotosNacionalesDeTodasLasVentasQueSeHicieron += $colV[$i]->retornarTotalVentaNacional();
            }
        }
        return $precioVentasMotosNacionalesDeTodasLasVentasQueSeHicieron;
    }
     ...
     * (*IMPORTANTE: invocar a los métodos implementados en la clase venta cuando crea necesario)
     * @return ARRAY $arrayVentasMotosImportadas
     */
    public function informarVentasImportadas(){
        $colV = $this->getColVenta();
        $arrayVentasDeMotosImportadasDeTodasLasVentasQueSeHicieron = [];

        //Uso for porque se cuantos ciclos hara
        for($i = 0; $i < count($colV); $i++){
            $arrayVentasImportadas = $colV[$i]->retornarMotosImportadas();

            array_push($arrayVentasDeMotosImportadasDeTodasLasVentasQueSeHicieron, $arrayVentasImportadas);
        }
        return $arrayVentasDeMotosImportadasDeTodasLasVentasQueSeHicieron;
    }
}